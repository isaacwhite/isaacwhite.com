...
String.prototype.normalizeUnicode = function() {
	var normalizedString = "";
	for(var i = 0; i<this.length; i++) {
		var concatCharCode = "";
		if(i != this.length-1) { //if we don't look at the last element
			concatCharCode += this.charCodeAt(i);
			concatCharCode += this.charCodeAt(i+1); 
		} else {
			concatCharCode += this.charCodeAt(i);
		}

		// console.log(concatCharCode);
		switch(concatCharCode) {
			case "195164": //the javascript code for a
			  	normalizedString += String.fromCharCode(228);
			  	i++;
			  	break;
			case "195171": //the javascript code for e
			  	normalizedString += String.fromCharCode(235);
			  	i++;
			  	break;
			case "195175": //the javascript code for i
				normalizedString += String.fromCharCode(239);
				i++;
				break;
			case "195182": //the javascript code for o
				normalizedString += String.fromCharCode(246);
				i++;
				break;
			case "195188": //the javascript code for u
				normalizedString += String.fromCharCode(252);
				i++;
				break;
			default:
				normalizedString += this.charAt(i);//just add the character back.
			  	break;
		}
	}	
...
BloomDictionary.checkSpelling = function(someWord) {
	...
	if((BloomDictionary.extras.indexOf(cleanString) == -1) &&
	(BloomDictionary.extras.indexOf(someWord.normalizeUnicode()) == -1)){ //finally make sure it's not an added value.
		console.log(someWord.normalizeUnicode());
